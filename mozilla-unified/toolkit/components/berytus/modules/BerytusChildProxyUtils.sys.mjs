...
export const getChildActor = (browsingContext, actorName) => {
    const IS_CONTENT = Services.appinfo.processType == Services.appinfo.PROCESS_TYPE_CONTENT;
    if (!IS_CONTENT) {
        throw new Error('Unable to get child acttor; caller attempted to retrieve '
            + 'child actor from a non-content process');
    }
    const window = browsingContext.window;
    if (!window) {
        throw new Error('Unable to get child actor; window is not set.');
    }
    return window.windowGlobalChild.getActor(actorName);
};
/* CommonProxy: */
function createFunctionProxy(target, receiver, fn) {
 ...
 */
function CommonProxy(target, fill) {
...
export function PassthroughFallbackProxy(target, methods, actorName) {
    const proxy = CommonProxy(target, (target, prop, receiver) => {
        if (methods.indexOf(prop) !== -1) {
            throw new Error('PasshthroughFallbackProxy Error: ' +
                'Accessed property is not a method ' +
                '(only methods are proxied)');
        }
        const routine = prop;
        return async function proxyCall(...args) {
            const actor = getChildActor(target.browsingContext, actorName);
            const result = await actor.sendQuery(`${actorName}:${routine}`, args);
            return result;
        };
    });
    return proxy;
